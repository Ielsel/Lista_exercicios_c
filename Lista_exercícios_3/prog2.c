...
// ----------- Parte 1 -----------)
//Escreva um algoritmo que solicite a idade de n alunos ao usuário e ao final apresente o maior e o menor dos valores informados, a soma e a média aritmética FEITO
...
//	2.2.2) Média de idade dos homens e média de idade das mulheres FEITO
//	2.2.3) Mulher mais nova/velha, homem mais novo/velho

// ----------- Parte 3 -----------)
// 3.1 Ao final, gerar um relatório das idades ordenado de forma crescente

// ----------- Parte 4 -----------)
// 4.1) Solicitar o nome do aluno
// 4.2) Solicitar as 3 notas do aluno
// 4.3) Imprimir relatório de alunos aprovados e reprovados (considerando média = 5):
// Aluna 1: Maria - idade: 16 - Média 7.9
// Aluna 2: Ana - idade: 17 - Média 8.4
// Aluno 3: Lucas - idade: 17 - Méida 9.5
// Aluno 4: Carlos - idade: 15 - Média 8.7
...
do { // Controlando a entrada de alunos
	...
	do {
	  printf("\n Informe a idade de %s: (8 a 120) ", Alunos[qtdAlunos].nome);
	  scanf("%i", &Alunos[qtdAlunos].idade);
	} while (Alunos[qtdAlunos].idade > 120 | Alunos[qtdAlunos].idade < 8); 
		
	do {
	  printf(" Informe o sexo de %s: [M/F]", Alunos[qtdAlunos].nome);
   ...
   if(qtdAlunos==1) { //Esse abre chave é obrigatorio, pois tem mais de um comando
			vlrMenor = idade;
			vlrMaior = idade;	
   } //Esse fecha chave é obrigatorio, pois tem mais de um comando
	...
	soma = 0; // essa linha é obrigatória por causa da linha 121
	for (int n=1; n<=3; n++) {
		do{
			printf(" Informe a %d. nota: [0 a 10] ", n);
 		  scanf("%f", &Alunos[qtdAlunos].notas[n]);
 		  if (Alunos[qtdAlunos].notas[n] > 0 & Alunos[qtdAlunos].notas[n] <= 10)
 		    soma += Alunos[qtdAlunos].notas[n];
		} while (Alunos[qtdAlunos].notas[n] < 0 | 
		         Alunos[qtdAlunos].notas[n] > 10);
	}
  ...
  for (int x=1; x<=qtdAlunos; x++) {
	if (Alunos[x].aprovado) {
		if (Alunos[x].sexo == 'm' | Alunos[x].sexo == 'M') 
		  printf("\nAluno: %s - Idade %i - Media: %.2f", 
		  Alunos[x].nome, Alunos[x].idade, Alunos[x].media);
	  else 
		  printf("\nAluna: %s - Idade %i - Media: %.2f", 
		  Alunos[x].nome, Alunos[x].idade, Alunos[x].media);
  }
}
  printf("\n");
